...
# If we compiled successfuly, link a library.
#
# The first argument should be the name of the library to build.
...
#
# example:   build-lib.sh foo.a ./objs/ ./foo/src/
#
# The argument immediately after the target name is the dir to put object files
# in.
...
ccflags="-Wall -Wno-endif-labels -g -DENABLE_LLVM=0 -falign-functions=4"
ccflags="$ccflags -D__STDC_LIMIT_MACROS -D__STDC_CONSTANT_MACROS"

# add include references for any devenv libraries the runtime library may use
# we will extend this var later with all the project source directories
include="-I$DEVENV_DIR/include/"

# add linker reference to any static libraries we may depend upon
libs=""
...
# If any of those dependencies are newer than the obj file, well delete it,
# which will cause it to be recompiled during the next loop.
...
# compile, every time we change the source file, or every time we change one
# of the source file's dependencies.
errorfound=0
...
if (( !errorfound )) ; then
	echo "linking..."
	# use ar to package the .o files into an archive
	if ar rcs $outfile $libs $(find $objdir -name "*.o"); then
		echo "done."
	else
		errorfound=1
	fi
